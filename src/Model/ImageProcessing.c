...
/*percentage*/
static unsigned char enlarge_compute_intensity_percentage(IMAGE * image, int x_big, int y_big, int percentage, int intensity_type){
	double x_small = x_big * 100.0 / percentage;
	double y_small = y_big * 100.0 / percentage;
	unsigned char temp[4];
	unsigned char return_value;	
	double x_factor = 1 - fmod(x_small, 1.0); double y_factor = 1 - fmod(y_small, 1.0);
	switch (intensity_type){
		case 0:
			temp[0] = GetPixelR(image, floor(x_big * 100 / percentage), floor(y_big * 100 / percentage));
			temp[1] = GetPixelR(image, floor(x_big * 100 / percentage), ceil(y_big * 100 / percentage));
			temp[2] = GetPixelR(image, ceil(x_big * 100 / percentage), floor(y_big * 100 / percentage));
			temp[3] = GetPixelR(image, ceil(x_big * 100 / percentage), ceil(y_big * 100 / percentage));
			break;
		case 1:
			temp[0] = GetPixelG(image, floor(x_big * 100 / percentage), floor(y_big * 100 / percentage));
			temp[1] = GetPixelG(image, floor(x_big * 100 / percentage), ceil(y_big * 100 / percentage));
			temp[2] = GetPixelG(image, ceil(x_big * 100 / percentage), floor(y_big * 100 / percentage));
			temp[3] = GetPixelG(image, ceil(x_big * 100 / percentage), ceil(y_big * 100 / percentage));
			break;
		case 2:
			temp[0] = GetPixelB(image, floor(x_big * 100 / percentage), floor(y_big * 100 / percentage));
			temp[1] = GetPixelB(image, floor(x_big * 100 / percentage), ceil(y_big * 100 / percentage));
			temp[2] = GetPixelB(image, ceil(x_big * 100 / percentage), floor(y_big * 100 / percentage));
			temp[3] = GetPixelB(image, ceil(x_big * 100 / percentage), ceil(y_big * 100 / percentage));
			break;
	}
...
IMAGE * ResizePercentage(IMAGE *image, unsigned int percentage){
	unsigned int out_height = percentage * image->Height / 100;
	unsigned int out_width = percentage * image->Width / 100;
	IMAGE * image_out = CreateImage(out_width, out_height);
	if (image_out == NULL) {printf("ERROR: Out of memory\n"); return image;}
	int i,j;
	...
	for (i = 0; i < image_out->Width; i++){
		for (j = 0; j < image_out->Height; j++){			
			if (percentage > 100){
				/*enlarge*/
				SetPixelR(image_out, i, j, enlarge_compute_intensity_percentage(image, i, j, percentage, 0));
				SetPixelG(image_out, i, j, enlarge_compute_intensity_percentage(image, i, j, percentage, 1));
				SetPixelB(image_out, i, j, enlarge_compute_intensity_percentage(image, i, j, percentage, 2));
